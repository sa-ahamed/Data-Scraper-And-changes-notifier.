#!/usr/bin/env python3
"""
site_crawler.py (Improved Version)

Multi-threaded website crawler + change tracker + formatted email notifier.

Features:
  • Multi-threaded crawling under same domain.
  • Respects allow/block lists.
  • Saves each page as a single Markdown file.
  • Detects content changes and generates diffs.
  • Sends well-formatted email summary (HTML + text).
  • Prints real-time progress suitable for cron logs.
"""
...
def looks_like_html(url: str, headers: Optional[dict]) -> bool:
    parsed = urllib.parse.urlparse(url)
...
def fetch_page(url: str) -> Tuple[Optional[str], Optional[requests.Response]]:
    last_exc = None
    for attempt in range(1, MAX_RETRY + 1):
        try:
            ...
            time.sleep(1)
    log(f"Failed to fetch {url}: {last_exc}")
    return None, None
    ...
    def worker():
        while True:
            ...
            with visited_lock:
                ...
                visited.add(url)
            log(f"Fetching: {url}")
            html_text, resp = fetch_page(url)
            if not resp or not looks_like_html(url, resp.headers):
                q.task_done()
                continue
